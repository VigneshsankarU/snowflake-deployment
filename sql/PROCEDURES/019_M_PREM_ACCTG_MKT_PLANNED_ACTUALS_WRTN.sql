...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_PREM_ACCTG_MKT_PLANNED_ACTUALS_WRTN("PARAM_JSON" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' 
...
SELECT MTRC, ST_CD, SEGMENT, LOB_GRP, PRD, YR, ACTUAL_SUM, Sum(PLND_AMT) AS PLND_AMT

FROM

(

SELECT EXTRACTED.MTRC AS MTRC, EXTRACTED.ST_CD AS ST_CD, EXTRACTED.LOB_GRP AS LOB_GRP, EXTRACTED.SEGMENT AS SEGMENT, EXTRACTED.PRD AS PRD, EXTRACTED.YR AS YR, EXTRACTED.ACTUAL_SUM AS ACTUAL_SUM, EXTRACTED.PLND_AMT AS PLND_AMT

   FROM ( SELECT ACTUALS.MTRC, ACTUALS.ST_CD, ACTUALS.LOB_GRP, ACTUALS.SEGMENT, ACTUALS.PRD, ACTUALS.YR, ACTUALS.ACTUAL_SUM,   PLANNED.PLND_AMT*-1 as plnd_amt

                      FROM (

/* WRITTEN  */
                         SELECT WRTN.MTRC, WRTN.ST_CD, WRTN.LOB_GRP, WRTN.SEGMENT, WRTN.PRD, WRTN.YR,  SUM(WRTN.ACTUAL_AMT) AS ACTUAL_SUM 

                            FROM (

                               SELECT ''WRTN'' AS MTRC,  ST_CD, 
...
WHEN LOB_CD IN (''HOMEOWNER/MANUF HOME'',''FIRE'',''FARMOWNER'',''FARMOWNERS'',''FCL'',''Floater'') THEN ''PROPERTY'' /* Added ''FARMOWNERS'',''FCL'',''Floater'' as part of EIM-49071,EIM-50215 */
                               ...
                               FROM    DB_V_PROD_ACT.W_PREMIUM_INFO_ACT
                                     ...
                                     WHERE ACCT_CD IN (400050,400053,400054,400055,400064,400074,400100,400150,400200,400314,402300,402302,402304,400250,400253,400254,400264,400274,400300,400303,400324,401250,401300,402310,402204,402205) AND 
...
SELECT ''WRTN'' AS MTRC,
                   ...
                   CAST(YEAR1 AS INTEGER) AS YR, 

                   SUM(MONETARY_AMOUNT * -1) AS ACTUAL_AMT

    FROM    DB_T_CORE_FIN_PROD.FIN_MANUAL_ENTRIES_PREMIUM_TXN

                   WHERE ACCOUNT1 IN (400050,400053,400054,400055,400064,400074,400100,400150,400200,400314,402300,402302,402304,400250,400253,400254,400264,400274,400300,400303,400324,401250,401300,402310,402204,402205) AND 
...
AND DB_T_SHRD_PROD.STATE IN (''AL'',''GA'',''MS'')
         ...
         GROUP BY 2,3,4,5,6)WRTN

          GROUP BY 1,2,3,4,5,6

)ACTUALS

JOIN DB_T_CORE_FIN_PROD.PLANNED_PREM PLANNED 

    ON PLANNED.MTRC = ACTUALS.MTRC

    AND PLANNED.ST_CD = ACTUALS.ST_CD

    AND PLANNED.LOB_GRP = ACTUALS.LOB_GRP

    AND PLANNED.SEGMENT = ACTUALS.SEGMENT

    AND PLANNED.PRD = ACTUALS.PRD

    AND PLANNED.YR = ACTUALS.YR
...
GROUP BY MTRC, ST_CD, SEGMENT, LOB_GRP, PRD, YR, ACTUAL_SUM

ORDER BY  YR, PRD, ST_CD, LOB_GRP, SEGMENT
) SRC
)
);
...
-- Component PREM_ACCTG_MKT_PLANNED_ACTUALS_WRTN, Type TARGET_EXPORT_PREPARE Stage data before exporting
CREATE OR REPLACE TEMPORARY TABLE PREM_ACCTG_MKT_PLANNED_ACTUALS_WRTN AS
(
SELECT
...
-- Component PREM_ACCTG_MKT_PLANNED_ACTUALS_WRTN, Type EXPORT_DATA Exporting data
;
