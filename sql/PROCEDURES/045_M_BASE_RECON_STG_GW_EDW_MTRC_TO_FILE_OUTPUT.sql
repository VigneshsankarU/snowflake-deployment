...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BASE_RECON_STG_GW_EDW_MTRC_TO_FILE_OUTPUT("PARAM_JSON" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' BEGIN 
...
CASE WHEN v_base_recon_mtrc_thresholdcheck IS NULL THEN 0 ELSE v_base_recon_mtrc_thresholdcheck END as v_thresholdcheck_temp,
CASE WHEN IFNULL(TRY_TO_DECIMAL(SQ_RECON_MTRC.DIFF_MTRC_VAL_OVR_THRSH), 0) > 0.00 THEN 1 ELSE v_thresholdcheck_temp END as v_base_recon_mtrc_thresholdcheck,
v_base_recon_mtrc_thresholdcheck as v_base_recon_mtrc_thresholdcheck_Final,
...
-- Component RECON_MTRC_OUTPUT_FILE_STG_GW_EDW, Type TARGET_EXPORT_PREPARE Stage data before exporting
CREATE OR REPLACE TEMPORARY TABLE RECON_MTRC_OUTPUT_FILE_STG_GW_EDW AS
(
SELECT
...
-- Component RECON_MTRC_OUTPUT_FILE_STG_GW_EDW, Type EXPORT_DATA Exporting data
;
