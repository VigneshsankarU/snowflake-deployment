...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BASE_WRK_FOP_BORDEREAU_ACCT_EXTRACT("WORKLET_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' 
...
TRANS_PREM_NET, CASE WHEN NAMED_INSRD LIKE ''%,%'' THEN REPLACE(NAMED_INSRD,'','','''') ELSE NAMED_INSRD END AS NAMED_INSRD, UW_CMPNY,POL_RSK_ST, POL_TYPE_DESC, TOT_PROP_PREM,LOB_CD 
...
RPAD ( SQ_FOP_BORDEREAU.UW_CMPNY , 3 , '' '' ) as out_UW_CMPNY_ACTG,
RPAD ( SQ_FOP_BORDEREAU.POL_RSK_ST , 2 , '' '' ) as out_POL_RSJ_ST_ACTG,
RPAD ( SQ_FOP_BORDEREAU.POL_TYPE_DESC , 3 , '' '' ) as out_POL_TYPE_DESC_ACTG,
...
CASE WHEN SQ_FOP_BORDEREAU.POL_TYPE_DESC = ''Farmowners'' OR SQ_FOP_BORDEREAU.POL_TYPE_DESC = ''MACHINERY'' OR SQ_FOP_BORDEREAU.POL_TYPE_DESC = ''FCL'' THEN ''FOP'' ELSE SQ_FOP_BORDEREAU.POL_TYPE_DESC END as v_POL_TYPE_DESC,
SQ_FOP_BORDEREAU.LOB_CD as out_Lob_cd,
