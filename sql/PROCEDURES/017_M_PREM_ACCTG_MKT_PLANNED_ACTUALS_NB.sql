...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_PREM_ACCTG_MKT_PLANNED_ACTUALS_NB("PARAM_JSON" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' 
...
(SELECT NB.MTRC, NB.ST_CD, NB.LOB_GRP, NB.SEGMENT, NB.PRD, NB.YR, SUM(NB.ACTUAL_AMT) AS ACTUAL_AMT FROM (

SELECT ''NB'' AS MTRC, ST_CD,
...
WHERE ACCT_CD IN (400100,400064,402300,400050,400054,400314) AND 
...
WHERE ACCOUNT1 IN (400100,400064,402300,400050,400054,400314) AND 
...
-- Component PREM_ACCTG_MKT_PLANNED_ACTUALS_NB, Type TARGET_EXPORT_PREPARE Stage data before exporting
CREATE OR REPLACE TEMPORARY TABLE PREM_ACCTG_MKT_PLANNED_ACTUALS_NB AS
(
SELECT
...
-- Component PREM_ACCTG_MKT_PLANNED_ACTUALS_NB, Type EXPORT_DATA Exporting data
;
