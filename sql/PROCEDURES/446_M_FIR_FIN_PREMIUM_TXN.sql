...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_FIR_FIN_PREMIUM_TXN("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
...
PMSTG_FIN_FIRTABLENAME varchar;
REPOS_CD varchar;
BEGIN 

PMMAPPINGNAME:=''M_FIR_FIN_PREMIUM_TXN''; 
PMSTG_FIN_FIRTABLENAME:='' ''; 
REPOS_CD:='' ''; 
...
-- Component sq_STG_FIN_FIR, Type SOURCE 
CREATE OR REPLACE TEMPORARY TABLE sq_STG_FIN_FIR AS
(
...
SELECT TRIM(STG_FIN_FIR.BUSINESS_UNIT), TRIM(STG_FIN_FIR.LEDGER), STG_FIN_FIR.FISCAL_YEAR , STG_FIN_FIR.ACCOUNTING_PERIOD , TRIM(STG_FIN_FIR.ACCOUNT_NBR), TRIM(STG_FIN_FIR.DEPTID), TRIM(STG_FIN_FIR.PRODUCT), TRIM(STG_FIN_FIR.PROJECT_ID), TRIM(STG_FIN_FIR.CHARTFIELD1), TRIM(STG_FIN_FIR.CHARTFIELD2), TRIM(STG_FIN_FIR.CHARTFIELD3), TRIM(STG_FIN_FIR.ALF_POL_PREFIX), TRIM(STG_FIN_FIR.ALF_POL_NUMBER), TRIM(STG_FIN_FIR.ALF_AGENT_NO), TRIM(STG_FIN_FIR.ALF_SERV_CTR_NBR), TRIM(STG_FIN_FIR.ALF_COMPANY_CD), TRIM(STG_FIN_FIR.GL_DISTRIB_STATUS), STG_FIN_FIR.MONETARY_AMOUNT, STG_FIN_FIR.ALF_POL_EFF_DATE, STG_FIN_FIR.ALF_POL_EXP_DATE,
STG_FIN_FIR.ALF_TOT_PREM,
TRIM(STG_FIN_FIR.ALF_TERM)
FROM
 DB_T_STAG_FIN_PROD.STG_FIN_FIR
WHERE
GL_DISTRIB_STATUS=''D''
) SRC
)
...
(
SELECT
substr ( sq_STG_FIN_FIR.BUSINESS_UNIT , 1 , 3 ) as CMPY_CD,
sq_STG_FIN_FIR.CHARTFIELD1 as ST_CD,
CASE WHEN try_cast(sq_STG_FIN_FIR.ALF_SERV_CTR_NBR as int)>0 THEN TO_CHAR ( TO_NUMBER(sq_STG_FIN_FIR.ALF_SERV_CTR_NBR) ) ELSE sq_STG_FIN_FIR.ALF_SERV_CTR_NBR END as out_SVC_NBR,
CASE WHEN try_cast(sq_STG_FIN_FIR.ALF_AGENT_NO as int)>0 THEN TO_CHAR ( TO_NUMBER(sq_STG_FIN_FIR.ALF_AGENT_NO) ) ELSE sq_STG_FIN_FIR.ALF_AGENT_NO END as out_AGNT_NBR,
sq_STG_FIN_FIR.ALF_COMPANY_CD as CMPY_TYPE_CD,
CASE WHEN LENGTH ( LTRIM ( RTRIM ( sq_STG_FIN_FIR.ALF_POL_PREFIX ) ) ) = 1 THEN LTRIM ( RTRIM ( sq_STG_FIN_FIR.ALF_POL_PREFIX ) ) || ''00000'' ELSE LTRIM ( RTRIM ( sq_STG_FIN_FIR.ALF_POL_PREFIX ) ) || ''0000'' END as var_PRODUCT_CD,
CASE WHEN LTRIM ( RTRIM ( sq_STG_FIN_FIR.PRODUCT ) ) IS NULL OR LTRIM ( RTRIM ( sq_STG_FIN_FIR.PRODUCT ) ) = '''' THEN ''A00000'' ELSE CASE WHEN LTRIM ( RTRIM ( sq_STG_FIN_FIR.PRODUCT ) ) = ''NO_PRD'' THEN CASE WHEN sq_STG_FIN_FIR.ALF_POL_PREFIX IN ( LTRIM ( RTRIM ( sq_STG_FIN_FIR.ALF_POL_PREFIX ) ) , ''G'' , ''N'' ) = 1 THEN ''A00000'' ELSE CASE WHEN LTRIM ( RTRIM ( sq_STG_FIN_FIR.ALF_POL_PREFIX ) ) = ''B'' THEN ''C00001'' ELSE var_PRODUCT_CD END END ELSE sq_STG_FIN_FIR.PRODUCT END END as out_PRODUCT_CD,
CONCAT ( sq_STG_FIN_FIR.ALF_POL_PREFIX , TO_CHAR ( sq_STG_FIN_FIR.ALF_POL_NUMBER ) ) as PLCY_NBR,
sq_STG_FIN_FIR.ALF_POL_EFF_DATE as PLCY_EFF_DT,
sq_STG_FIN_FIR.ALF_POL_EXP_DATE as PLCY_EXP_DT,
CASE WHEN sq_STG_FIN_FIR.ALF_TERM IS NULL OR sq_STG_FIN_FIR.ALF_TERM = '''' OR TRIM(sq_STG_FIN_FIR.ALF_TERM) = '''' THEN NULL ELSE TO_NUMBER(sq_STG_FIN_FIR.ALF_TERM) END as PLCY_TERM,
sq_STG_FIN_FIR.ACCOUNTING_PERIOD as ACCTNG_MO,
sq_STG_FIN_FIR.FISCAL_YEAR as ACCTNG_YR,
sq_STG_FIN_FIR.FISCAL_YEAR * 100 + sq_STG_FIN_FIR.ACCOUNTING_PERIOD as MO_ID,
sq_STG_FIN_FIR.CHARTFIELD2 as POOL_CD,
sq_STG_FIN_FIR.CHARTFIELD3 as REINSUR_CD,
sq_STG_FIN_FIR.ALF_TOT_PREM as TOT_PREM_AMT,
sq_STG_FIN_FIR.MONETARY_AMOUNT as MONETARY_AMT,
sq_STG_FIN_FIR.LEDGER as LEDGER,
sq_STG_FIN_FIR.ACCOUNT_NBR as ACCT_NBR,
sq_STG_FIN_FIR.GL_DISTRIB_STATUS as GL_DSTR_STATUS,
sq_STG_FIN_FIR.DEPTID as DEPT_ID,
sq_STG_FIN_FIR.PROJECT_ID as PROJ_ID,
NULL as ALF_ACCT_ID,
:PMMappingName as out_PRGM_NM,
:ACT_PROJ_ID as out_ACT_PROJ_ID,
:ECTL_ACTIVE_IND as out_BATCH_ACTIVE_IND,
:PMSTG_FIN_FIRTableName as out_TABLE_NM,
:REPOS_CD as out_REPOS_CD,
:ACT_PROJ_ID as IN_ACT_PROJ_ID,
:ECTL_ACTIVE_IND as IN_BATCH_ACTIVE_IND,
''m_STG_FIN_CLM'' IN_PRGM_NM,
null IN_TABLE_NM,
sq_STG_FIN_FIR.source_record_id
FROM
sq_STG_FIN_FIR
);
...
-- Component FIN_PREMIUM_TXN_FIR_INS, Type TARGET 
