...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BAL_SRC_TO_STG_BENFIELD_TABLES("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' DECLARE TABLE_NM varchar;
...
SELect COUNT(*) AS POL_NBR FROM TABLE_NM
) SRC
)
);
...
-- Component SQ_TRG_BENFIELD_BASE_SRC, Type TABLE_DDL Creating an empty table
CREATE OR REPLACE TEMPORARY TABLE SQ_TRG_BENFIELD_BASE
(
RECORD_CNT decimal,
...
-- Component SQ_TRG_BENFIELD_BASE_SRC, Type IMPORT_DATA Importing Data
;
...
(
SELECT
TO_NUMBER(SQ_TRG_BENFIELD_BASE.RECORD_CNT) as out_RECORD_CNT,
vROW_NO + 1 as vROW_NO,
vROW_NO as ROW_NO,
1 as out_DUMMY,
SQ_TRG_BENFIELD_BASE.source_record_id
FROM
SQ_TRG_BENFIELD_BASE
);
