...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BASE_PLCY_CVGE_MTRC_GL_INSUPD("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS '
BEGIN
  ...
  (
         SELECT
                ...
                $4  AS cov_type_cd,
                $5  AS trns_writ_prem_amt,
                $6  AS trns_earn_prem_amt,
                $7  AS trns_unearn_prem_amt,
                $8  AS gl_extr_mo,
                $9  AS gl_extr_yr,
  ...
  (
         ...
         FROM   (
                          ...
                          SELECT    lkp_agmt.agmt_id                                  AS agmt_id_in,
                                    lkp_feat.feat_id                                  AS feat_id_in,
...
WITH base AS (
  ...
  LEFT JOIN lkp_feat ON lkp_agmt.source_record_id = lkp_feat.source_record_id
)
SELECT
...
FROM base;
  ...
  -- Component LKP_PLCY_CVGE_MTRC_GL, Type LOOKUP
  CREATE
  OR
  replace TEMPORARY TABLE lkp_plcy_cvge_mtrc_gl AS
  (
            SELECT    lkp.agmt_id,
                      lkp.feat_id,
                      lkp.insrnc_mtrc_type_cd,
                      lkp.plcy_cvge_mtrc_strt_dttm,
                      lkp.plcy_cvge_mtrc_end_dttm,
                      lkp.plcy_cvge_amt,
                      exp_data_cleansing1.source_record_id,
                      row_number() over(PARTITION BY exp_data_cleansing1.source_record_id ORDER BY lkp.agmt_id ASC,lkp.feat_id ASC,lkp.insrnc_mtrc_type_cd ASC,lkp.plcy_cvge_mtrc_strt_dttm ASC,lkp.plcy_cvge_mtrc_end_dttm ASC,lkp.plcy_cvge_amt ASC) rnk
            ...
            left join
                      (
                             SELECT agmt_id                  AS agmt_id,
                                    ...
                                    plcy_cvge_mtrc_end_dttm  AS plcy_cvge_mtrc_end_dttm,
                             ...
                             WHERE  insrnc_mtrc_type_cd IN
                                    (
                                           ...
                                           AND    src_idntftn_sys=''DS''
                                           AND    expn_dt=''9999-12-31'')
                             AND    plcy_cvge_mtrc_strt_dttm<plcy_cvge_mtrc_end_dttm ) lkp
            ...
            AND       lkp.plcy_cvge_amt = exp_data_cleansing1.trns_prem_amt
            AND       lkp.plcy_cvge_mtrc_strt_dttm = exp_data_cleansing1.out_gl_start_dttm
            AND       lkp.plcy_cvge_mtrc_end_dttm = exp_data_cleansing1.out_gl_end_dttm qualify rnk = 1 );
  ...
  (
             SELECT     exp_data_cleansing1.agmt_id             AS agmt_id,
                     ...
                     OR         exp_data_cleansing1.feat_id IS NULL , ''R'' ,
                                lkp_plcy_cvge_mtrc_gl.agmt_id IS NULL , ''I'' ,
                                NOT (
                                           lkp_plcy_cvge_mtrc_gl.agmt_id IS NULL )
                     AND        lkp_plcy_cvge_mtrc_gl.plcy_cvge_amt <> exp_data_cleansing1.trns_prem_amt , ''U'' ,
                                ''R'' )                                                         AS out_flg,
                        current_timestamp                                                     AS out_session_start_dttm,
                        to_timestamp_ntz ( ''1900-01-01 00:00:00.000000'' , ''YYYY-MM-DD HH24:MI:SS.FF6'' ) AS out_start_dttm,
                        to_timestamp_ntz ( ''9999-12-31 23:59:59.999999'' , ''YYYY-MM-DD HH24:MI:SS.FF6'' ) AS out_end_dttm,
                        ''UNK''                                                                 AS out_unknown,
                        $prcs_id                                                              AS out_prcs_id,
                        exp_data_cleansing1.source_record_id
             FROM       exp_data_cleansing1
             inner join lkp_plcy_cvge_mtrc_gl
             ON         exp_data_cleansing1.source_record_id = lkp_plcy_cvge_mtrc_gl.source_record_id );
  ...
  SELECT fil_rejects.agmt_id             AS agmt_id,
         fil_rejects.feat_id             AS feat_id,
         fil_rejects.start_dttm          AS agmt_feat_strt_dttm,
         fil_rejects.unknown             AS agmt_feat_role_cd,
         fil_rejects.insrnc_mtrc_type_cd AS insrnc_mtrc_type_cd,
         fil_rejects.gl_start_dttm       AS plcy_cvge_mtrc_strt_dttm,
         fil_rejects.gl_end_dttm         AS plcy_cvge_mtrc_end_dttm,
         fil_rejects.unknown             AS tm_prd_cd,
         fil_rejects.trns_prem_amt       AS plcy_cvge_amt,
  ...
  (
         ...
         FROM   (
                         ...
                         FROM     (
                                           ...
                                           WHERE    1=2
                                           ORDER BY 1 ) src ) );
  -- Component PLCY_CVGE_MTRC_post_sql, Type TARGET
  INSERT INTO db_t_prod_core.plcy_cvge_mtrc
              (
                          agmt_id
              )
  ...
  -- PIPELINE END FOR 2
  -- Component PLCY_CVGE_MTRC_post_sql, Type Post SQL
  UPDATE db_t_prod_core.plcy_cvge_mtrc as tgt
  SET    plcy_cvge_mtrc_end_dttm = new_plcy_cvge_mtrc_end_dttm,
  ...
  FROM  
         (
                    SELECT     agmt_id ,
                               ...
                               edw_strt_dttm,
                               CASE
                                          WHEN row_number() over(PARTITION BY agmt_id , feat_id ,insrnc_mtrc_type_cd, plcy_cvge_mtrc_strt_dttm ORDER BY edw_strt_dttm DESC) <> 1 THEN plcy_cvge_mtrc_strt_dttm - interval ''0.000001 second''
                                          ELSE plcy_cvge_mtrc_end_dttm
                               END                                                                                                                                                                                                        new_plcy_cvge_mtrc_end_dttm ,
                               max(edw_strt_dttm) over(PARTITION BY agmt_id , feat_id ,insrnc_mtrc_type_cd ORDER BY plcy_cvge_mtrc_strt_dttm,edw_strt_dttm ASC ROWS BETWEEN 1 following AND        1 following) - interval ''0.000001 second'' AS new_edw_end_dttm
                    FROM       db_t_prod_core.plcy_cvge_mtrc                                                                                                                                                                                                AS p
                    ...
                    AND        t.expn_dt=''9999-12-31'' qualify new_plcy_cvge_mtrc_end_dttm <> plcy_cvge_mtrc_end_dttm
  ...
  AND    tgt.plcy_cvge_mtrc_strt_dttm = updt.plcy_cvge_mtrc_strt_dttm
