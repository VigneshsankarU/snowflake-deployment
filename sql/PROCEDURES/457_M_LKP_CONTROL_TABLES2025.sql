...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_LKP_CONTROL_TABLES2025("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' BEGIN 
...
exp_Maplet_Input
LEFT JOIN (
SELECT ECTL_PRGM_INFO.ECTL_PRGM_ID as ECTL_PRGM_ID, ECTL_PRGM_INFO.ECTL_PROJ_ID as ECTL_PROJ_ID, ECTL_PRGM_INFO.ECTL_PRGM_NM as ECTL_PRGM_NM FROM ECTL_PRGM_INFO
...
ECTL_PROJ_ID,
ECTL_ACTIVE_IND
FROM ECTL_BATCH_INFO
) LKP ON LKP.ECTL_PROJ_ID = exp_Maplet_Input.PROJECT_ID AND LKP.ECTL_ACTIVE_IND = exp_Maplet_Input.BATCH_ACTIVE_IND
QUALIFY RNK = 1
);
...
CREATE OR REPLACE TEMPORARY TABLE out_output_maplet AS
(
SELECT
