...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BASE_RECON_EDW_AN_PROD_MTRC_INS("WORKLET_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' DECLARE
...
FROM	
 ...
 WHERE RECON_EXTC_MTRC_INT.RECON_MTRC_LAYER =''EDW | ANPROD''
...
SELECT
  ...
  -- Source value formatted as string
  COALESCE(
    TO_CHAR(TRY_TO_DECIMAL(e.SOURCE_VAL::VARCHAR), ''9999999999999990.00''),
    ''0.00''
  ) AS V_SOURCE_SQL_VALUE,

  -- Copy for output (required per user)
  TO_CHAR(
    TRY_TO_DECIMAL(
      COALESCE(
        TO_CHAR(TRY_TO_DECIMAL(e.SOURCE_VAL::VARCHAR), ''9999999999999990.00''),
  ...
  -- Target value formatted as string
  COALESCE(
    TO_CHAR(TRY_TO_DECIMAL(e.TARGET_VAL::VARCHAR), ''9999999999999990.00''),
    ''0.00''
  ...
  -- Difference value
  TO_CHAR(
    TRY_TO_DECIMAL(V_SOURCE_SQL_VALUE::VARCHAR) - TRY_TO_DECIMAL(V_TARGET_SQL_VALUE::VARCHAR),
    ''9999999999999990.00''
  ) AS DIFF_SQL_VAL_VAR,

  TO_CHAR(TRY_TO_DECIMAL(DIFF_SQL_VAL_VAR::VARCHAR), ''9999999999999990.00'') AS DIFF_MTRC_VAL,
  ...
  -- Diff over threshold
  TO_CHAR(
    ABS(TRY_TO_DECIMAL(DIFF_SQL_VAL_VAR::VARCHAR)) - ABS(TRY_TO_DECIMAL(e.THRSH_VAL::VARCHAR)),
    ''9999999999999990.00''
  ...
  -- Prior diff value
  TO_CHAR(
    TRY_TO_DECIMAL(LKM.DIFF_MTRC_VAL::VARCHAR),
    ''9999999999999990.00''
  ...
  CASE
    WHEN (
      TRY_TO_DECIMAL(DIFF_SQL_VAL_VAR::VARCHAR) - TRY_TO_DECIMAL(LKM.DIFF_MTRC_VAL::VARCHAR)
    ) <> 0 THEN
      CASE
        WHEN TRY_TO_DECIMAL(LKM.DIFF_MTRC_VAL::VARCHAR) <> 0 THEN
          TO_CHAR(
            (
              (TRY_TO_DECIMAL(DIFF_SQL_VAL_VAR::VARCHAR) - TRY_TO_DECIMAL(LKM.DIFF_MTRC_VAL::VARCHAR))
              / TRY_TO_DECIMAL(LKM.DIFF_MTRC_VAL::VARCHAR)
      ...
      END
    ELSE ''0.000''
  END AS PCT_CHG_DIFF_MTRC_VAL_VAR,

  COALESCE(PCT_CHG_DIFF_MTRC_VAL_VAR, ''0.000'') AS PCT_CHG_DIFF_MTRC_VAL,
...
JOIN LKP_RECON_MTRC LKM
  ON e.source_record_id = LKM.source_record_id;



-- Component RECON_MTRC, Type TARGET 
