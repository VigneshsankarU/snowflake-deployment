...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BASE_CNSUS_BLCK_GRP_INSUPD("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' declare
...
,max(createtime),cast (0 as bigint) Retired,TerritoryFIPSCode_alfa  from (

SELECT  pctl_state.TYPECODE_stg as state_TYPECODE, pctl_country.TYPECODE_stg as ctry_TYPECODE, pc_address.county_stg as county, pc_address.createtime_stg as createtime,CASE WHEN LENGTH(TRIM(TerritoryFIPSCode_alfa_stg))=15 THEN TerritoryFIPSCode_alfa_stg ELSE NULL END AS TerritoryFIPSCode_alfa

FROM  (SELECT	pc_address.CreateTime_stg, pc_address.County_stg, pc_address.State_stg, pc_address.UpdateTime_stg, pc_address.Country_stg, TerritoryFIPSCode_alfa_stg

FROM DB_T_PROD_STAG.pc_address WHERE pc_address.UpdateTime_stg> (:start_dttm) and	pc_address.UpdateTime_stg <= (:end_dttm)) pc_address left join (SELECT TYPECODE_stg,ID_stg FROM DB_T_PROD_STAG.pctl_country) pctl_country on pctl_country.ID_stg=pc_address.country_stg join (SELECT TYPECODE_stg,ID_stg FROM DB_T_PROD_STAG.pctl_state) pctl_state on pc_address.State_stg=pctl_state.ID_stg

where pc_address.county_stg is not null and  TerritoryFIPSCode_alfa is not null
...
SELECT	 pctl_state.TYPECODE_stg as state_TYPECODE, pctl_country.TYPECODE_stg as ctry_TYPECODE,

		pc_policylocation.CountyInternal_stg
        --(NOT CASESPECIFIC) 
        AS County,pc_policylocation.createtime_stg as createtime,CASE WHEN LENGTH(TRIM(TerritoryFIPSCode_alfainternal_stg))=15 THEN TerritoryFIPSCode_alfainternal_stg ELSE NULL END AS TerritoryFIPSCode_alfa

FROM	 (SELECT pc_policylocation.CountyInternal_stg, pc_policylocation.CreateTime_stg, pc_policylocation.StateInternal_stg, pc_policylocation.TerritoryFIPSCode_alfainternal_stg,CountryInternal_stg

FROM DB_T_PROD_STAG.pc_policylocation where pc_policylocation.UpdateTime_stg > (:start_dttm) and pc_policylocation.UpdateTime_stg <= (:end_dttm)) pc_policylocation join (SELECT TYPECODE_stg,ID_stg FROM DB_T_PROD_STAG.pctl_state) pctl_state 

	on	 pc_policylocation.StateInternal_stg=pctl_state.ID_stg left join (SELECT TYPECODE_stg,ID_stg FROM DB_T_PROD_STAG.pctl_country) pctl_country 

	on	pctl_country.ID_stg=pc_policylocation.CountryInternal_stg

where	pc_policylocation.CountyInternal_stg is not null and  TerritoryFIPSCode_alfa is not null

) a where	TerritoryFIPSCode_alfa is not null

group by state_TYPECODE,ctry_TYPECD,county(NOT CASESPECIFIC),TerritoryFIPSCode_alfa
) SRC
)
);
...
LTRIM ( RTRIM ( CASE WHEN sq_pc_address.TerritoryFIPSCode_alfa IS NULL THEN sq_pc_address.TerritoryFIPSCode_alfa ELSE substr ( sq_pc_address.TerritoryFIPSCode_alfa , 6 , 6 ) END ) ) as v_TerritoryFIPSCode_alfa,
...
LKP_4.CNSUS_TRCT_ID /* replaced lookup LKP_CNSUS_TRCT */ as v_CNSUS_TRCT_ID,
CASE WHEN v_CNSUS_TRCT_ID IS NULL THEN 9999 ELSE v_CNSUS_TRCT_ID END as o_CNSUS_TRCT_ID,
sq_pc_address.CreateTime as CreateTime,
sq_pc_address.Retired as Retired,
LTRIM ( RTRIM ( CASE WHEN sq_pc_address.TerritoryFIPSCode_alfa IS NULL THEN sq_pc_address.TerritoryFIPSCode_alfa ELSE substr ( sq_pc_address.TerritoryFIPSCode_alfa , 12 , 4 ) END ) ) as o_TerritoryFIPSCode_alfa,
...
LEFT JOIN LKP_CNSUS_TRCT LKP_4 ON LKP_4.CNTY_ID = v_cnty_id AND LKP_4.GEOGRCL_AREA_SHRT_NAME = ltrim ( rtrim ( sq_pc_address.County ) ) AND LKP_4.CNSUS_TRCT_NUM = v_TerritoryFIPSCode_alfa
QUALIFY RNK = 1
);


-- Component LKP_CNSUS_BLCK_GRP, Type LOOKUP 
...
LKP.LOCTR_SBTYPE_CD,
LKP.CNSUS_BLCK_GRP_NUM,
exp_pass_through_id.source_record_id,
ROW_NUMBER() OVER(PARTITION BY exp_pass_through_id.source_record_id ORDER BY LKP.CNSUS_BLCK_GRP_ID desc,LKP.CNSUS_TRCT_ID desc,LKP.GEOGRCL_AREA_SBTYPE_CD desc,LKP.GEOGRCL_AREA_SHRT_NAME desc,LKP.GEOGRCL_AREA_NAME desc,LKP.GEOGRCL_AREA_DESC desc,LKP.CURY_CD desc,LKP.GEOGRCL_AREA_STRT_DT desc,LKP.GEOGRCL_AREA_END_DT desc,LKP.LOCTR_SBTYPE_CD desc,LKP.CNSUS_BLCK_GRP_NUM desc,LKP.PRCS_ID desc,LKP.EDW_STRT_DTTM desc,LKP.EDW_END_DTTM desc) RNK
FROM
exp_pass_through_id
LEFT JOIN (
SELECT 
...
) LKP ON LKP.CNSUS_TRCT_ID = exp_pass_through_id.o_CNSUS_TRCT_ID AND LKP.CNSUS_BLCK_GRP_NUM = exp_pass_through_id.o_TerritoryFIPSCode_alfa AND LKP.GEOGRCL_AREA_SHRT_NAME = exp_pass_through_id.County
QUALIFY RNK = 1
);
...
LKP_CNSUS_BLCK_GRP.CNSUS_BLCK_GRP_ID as lkp_CNSUS_BLCK_GRP_ID,
exp_pass_through_id.o_TerritoryFIPSCode_alfa as TerritoryFIPSCode_alfa,
MD5 ( LKP_CNSUS_BLCK_GRP.GEOGRCL_AREA_SBTYPE_CD || LKP_CNSUS_BLCK_GRP.LOCTR_SBTYPE_CD ) as lkp_checksum,
MD5 ( v_geogrcl_sbtype || v_loctr_sbtype ) as in_checksum,
CASE WHEN LKP_CNSUS_BLCK_GRP.CNSUS_BLCK_GRP_ID IS NULL THEN ''I'' ELSE ( CASE WHEN lkp_checksum <> in_checksum THEN ''U'' ELSE ''R'' END ) END as o_upd_or_ins,
CASE WHEN LKP_CNSUS_BLCK_GRP.CNSUS_BLCK_GRP_ID IS NULL THEN ''I'' ELSE ( CASE WHEN lkp_checksum <> in_checksum THEN ''U'' ELSE ''R'' END ) END as v_o_upd_or_ins,
CURRENT_TIMESTAMP as EDW_STRT_DTTM,
TO_DATE ( ''12/31/9999 23:59:59.999999'' , ''MM/DD/YYYY HH24:MI:SS.FF6'' ) as EDW_END_DTTM,
exp_pass_through_id.Retired as Retired,
CASE WHEN v_o_upd_or_ins = ''U'' THEN LKP_CNSUS_BLCK_GRP.CNSUS_TRCT_ID ELSE exp_pass_through_id.o_CNSUS_TRCT_ID END as o_CNSUS_TRCT_ID,
exp_pass_through_id.source_record_id,
row_number() over (partition by exp_pass_through_id.source_record_id order by exp_pass_through_id.source_record_id) as RNK
FROM
exp_pass_through_id
INNER JOIN LKP_CNSUS_BLCK_GRP ON exp_pass_through_id.source_record_id = LKP_CNSUS_BLCK_GRP.source_record_id
...
exp_SET_INS_UPD_FLAG.o_CNSUS_TRCT_ID as CNSUS_TRCT_ID,
exp_SET_INS_UPD_FLAG.lkp_CNSUS_BLCK_GRP_ID as lkp_CNSUS_BLCK_GRP_ID,
...
rtr_update_insert_CNSUS_INS.CNSUS_TRCT_ID as CNSUS_TRCT_ID,
rtr_update_insert_CNSUS_INS.lkp_CNSUS_BLCK_GRP_ID as lkp_CNSUS_BLCK_GRP_ID1,
...
CASE WHEN upd_insert_update_CNSUS.lkp_CNSUS_BLCK_GRP_ID1 IS NULL THEN row_number() over (order by 1) ELSE upd_insert_update_CNSUS.lkp_CNSUS_BLCK_GRP_ID1 END as o_NEXTVAL,
upd_insert_update_CNSUS.o_TerritoryFIPSCode_alfa1 as o_TerritoryFIPSCode_alfa1,
upd_insert_update_CNSUS.CNSUS_TRCT_ID as CNSUS_TRCT_ID,
...
-- Component CNSUS_BLCK_GRP, Type TARGET 
INSERT INTO DB_T_PROD_CORE.CNSUS_BLCK_GRP
(
CNSUS_BLCK_GRP_ID,
CNSUS_TRCT_ID,
...
SELECT
exp_pass_to_target_insert_pc111.o_NEXTVAL as CNSUS_BLCK_GRP_ID,
exp_pass_to_target_insert_pc111.CNSUS_TRCT_ID as CNSUS_TRCT_ID,
...
exp_pass_to_target_insert_pc111.o_TerritoryFIPSCode_alfa1 as CNSUS_BLCK_GRP_NUM,
...
FROM (
SELECT SRC.*, row_number() over (order by 1) AS source_record_id FROM (
select state_TYPECODE,case when (ctry_TYPECODE is null) then ''US'' else ctry_TYPECODE end ctry_TYPECD,county(NOT CASESPECIFIC),max(createtime),cast (0 as bigint) Retired,TerritoryFIPSCode_alfa  from (

SELECT  pctl_state.TYPECODE_stg as state_TYPECODE, pctl_country.TYPECODE_stg as ctry_TYPECODE, pc_address.county_stg as county,pc_address.createtime_stg as createtime,pc_address.TerritoryFIPSCode_alfa_stg as TerritoryFIPSCode_alfa

FROM  (SELECT	pc_address.CreateTime_stg, pc_address.County_stg, pc_address.State_stg, pc_address.UpdateTime_stg, pc_address.Country_stg, TerritoryFIPSCode_alfa_stg

FROM DB_T_PROD_STAG.pc_address WHERE pc_address.UpdateTime_stg> (:start_dttm) and	pc_address.UpdateTime_stg <= (:end_dttm)) pc_address left join (SELECT TYPECODE_stg,ID_stg FROM pctl_country) pctl_country on pctl_country.ID_stg=pc_address.Country_stg join (SELECT TYPECODE_stg,ID_stg FROM pctl_state) pctl_state on pc_address.State_stg=pctl_state.ID_stg
...
SELECT	 pctl_state.TYPECODE_stg as state_TYPECODE, pctl_country.TYPECODE_stg as ctry_TYPECODE,
		...
		pc_policylocation.CountyInternal_stg(NOT CASESPECIFIC) AS County,pc_policylocation.createtime_stg as createtime,pc_policylocation.TerritoryFIPSCode_alfainternal_stg as TerritoryFIPSCode_alfa

FROM	 (SELECT CountyInternal_stg,createtime_stg,TerritoryFIPSCode_alfainternal_stg,StateInternal_stg,CountryInternal_stg FROM DB_T_PROD_STAG.pc_policylocation) pc_policylocation join (SELECT TYPECODE_stg,ID_stg FROM DB_T_PROD_STAG.pctl_state) pctl_state 
...
(
SELECT
CASE WHEN sq_pc_address1.TerritoryFIPSCode_alfa IS NULL THEN ''0'' ELSE SUBSTR ( sq_pc_address1.TerritoryFIPSCode_alfa , 12 , 4 ) END as o_TerritoryFIPSCode_alfa,
...
-- Component CNSUS_BLCK_GRP1, Type TARGET 
INSERT INTO DB_T_PROD_CORE.CNSUS_BLCK_GRP
(
CNSUS_BLCK_GRP_NUM
)
SELECT
exp_pass_through_id1.o_TerritoryFIPSCode_alfa as CNSUS_BLCK_GRP_NUM
FROM
exp_pass_through_id1;


-- PIPELINE END FOR 2
-- Component CNSUS_BLCK_GRP1, Type Post SQL 
UPDATE  DB_T_PROD_CORE.CNSUS_BLCK_GRP  FROM  

(

SELECT	distinct   CNSUS_BLCK_GRP_ID,CNSUS_TRCT_ID,GEOGRCL_AREA_SHRT_NAME,GEOGRCL_AREA_STRT_DT,EDW_STRT_DTTM,CNSUS_BLCK_GRP_NUM,

max(EDW_STRT_DTTM) over (partition by  CNSUS_TRCT_ID,GEOGRCL_AREA_SHRT_NAME,CNSUS_BLCK_GRP_NUM ORDER BY EDW_STRT_DTTM ASC rows between 1 following and 1 following) - INTERVAL ''1 SECOND'' 

 as LEAD1

FROM 

DB_T_PROD_CORE.CNSUS_BLCK_GRP  

)  A

set EDW_END_DTTM=A.LEAD1

where  CNSUS_BLCK_GRP.CNSUS_BLCK_GRP_ID=A.CNSUS_BLCK_GRP_ID

AND CNSUS_BLCK_GRP.CNSUS_TRCT_ID=A.CNSUS_TRCT_ID

and CNSUS_BLCK_GRP.GEOGRCL_AREA_SHRT_NAME=A.GEOGRCL_AREA_SHRT_NAME

and CNSUS_BLCK_GRP.CNSUS_BLCK_GRP_NUM=A.CNSUS_BLCK_GRP_NUM

and CNSUS_BLCK_GRP.EDW_STRT_DTTM = A.EDW_STRT_DTTM

and CAST(CNSUS_BLCK_GRP.EDW_END_DTTM AS DATE)=''9999-12-31''
