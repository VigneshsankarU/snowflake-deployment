...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_ADDRESS2025("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
...
''ADDRESS'' as out_TABLE_NM,
''ADDRESS'' as IN_SRC_TABLE_NM,
...
call MPLT_LKP_CTRL_TBLS_VALIDATION( ''exp_Address'');
--DB_T_CORE_PROD.ADDRESS'' );
...
create or replace temp table RTR_ADDRESS_NEW_ADDRESS as
SELECT
...
FROM
exp_Address
LEFT JOIN MPLT_LKP_CTRL_TBLS_VALIDATION m_lkp_Control_Tables ON exp_Address.source_record_id = m_lkp_Control_Tables.source_record_id
