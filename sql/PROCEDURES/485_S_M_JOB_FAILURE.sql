...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.S_M_JOB_FAILURE("WORKLET_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS OWNER
AS '
...
BEGIN
  ...
  CALL sp_set_param(:v_run_id, ''1_session'', ''s_m_job_failure'', ''PRCS_ID'', :prcs_id);
  CALL sp_set_param(:v_run_id, ''1_session'', ''s_m_job_failure'', ''SUB_PRCS_NM'', :sub_prcs_nm);
  CALL sp_set_param(:v_run_id, ''1_session'', ''s_m_job_failure'', ''SRC_NM'', :src_nm);
  
  -- Call mapping stored procedure
  CALL m_job_failure(worklet_name);

  INSERT INTO control_status (run_id, worklet_name, task_name, task_status, task_start_dttm, task_end_dttm, var_json)
  SELECT :v_run_id, :worklet_name, ''s_m_job_failure'', ''SUCCEEDED'', :v_start_time, CURRENT_TIMESTAMP(), NULL;

EXCEPTION WHEN OTHER THEN
    INSERT INTO control_status (run_id, worklet_name, task_name, task_status, task_start_dttm, task_end_dttm, var_json)
      SELECT  :v_run_id, :worklet_name, ''s_m_job_failure'', ''FAILED'', :v_start_time, CURRENT_TIMESTAMP(), OBJECT_CONSTRUCT(''SQLERRM'', :sqlerrm);
END;
';