...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BAL_AMT_STG_TO_TGT("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
...
TGT_NM:='' ''; 
MAPPING_NM:=''M_BAL_AMT_STG_TO_TGT''; 
...
SELECT COALESCE(SUM(MONETARY_AMT * MULTIPLY_FACTOR),0) AS SUM_AMT FROM TGT_NM A,ECTL_BATCH_INFO B,ECTL_SRC_XREF C,ECTL_SRC_INST_XREF D,REPOS_FIN_ACCT_NBR_LKUP E
WHERE A.ECTL_BATCH_ID = B.ECTL_BATCH_ID AND 
A.ECTL_SRC_ID = C.ECTL_SRC_ID AND 
A.ECTL_SRC_INST_ID = D.ECTL_SRC_INST_ID AND 
C.ECTL_SRC_DESC = ''Peoplesoft Repository table'' AND
D.ECTL_SRC_INST_DESC = ''$REPOS_CD'' AND
B.ECTL_ACTIVE_IND = ''$ECTL_ACTIVE_IND'' AND
B.ECTL_PROJ_ID=''$ACT_PROJ_ID'' AND
E. REPOS_CD=''$REPOS_CD'' AND E.ACCT_NBR=A.ACCT_NBR
) SRC
)
);
...
(
SELECT
sq_TRG_FILE.SUM_AMT as SUM_MONETARY_AMT_TRG,
1 as DUMMY,
...
(
SELECT
sq_TGT_TABLE.MONETARY_AMT as SUM_MONETARY_AMT_TGT,
1 as DUMMY,
...
(
SELECT
exp_DUMMY_PORT_TRG.SUM_MONETARY_AMT_TRG as SUM_MONETARY_AMT_TRG,
exp_DUMMY_PORT_TRG.DUMMY as DUMMY,
exp_DUMMY_PORT_TGT.SUM_MONETARY_AMT_TGT as SUM_MONETARY_AMT_TGT,
...
-- Component exp_UNBAL_AMT, Type EXPRESSION 
CREATE OR REPLACE TEMPORARY TABLE exp_UNBAL_AMT AS
(
SELECT
ROUND ( jnr_TRG_TGT.SUM_MONETARY_AMT_TGT , 0 ) as out_SUM_MONE_AMT_TGT,
ROUND ( jnr_TRG_TGT.SUM_MONETARY_AMT_TRG , 0 ) as out_SUM_MONE_AMT_TRG,
ROUND ( jnr_TRG_TGT.SUM_MONETARY_AMT_TRG , 0 ) - ROUND ( jnr_TRG_TGT.SUM_MONETARY_AMT_TGT , 0 ) as out_UNBAL_AMT,
...
CASE WHEN ( ROUND ( jnr_TRG_TGT.SUM_MONETARY_AMT_TRG , 0 ) - ROUND ( jnr_TRG_TGT.SUM_MONETARY_AMT_TGT , 0 ) ) <> 0 THEN TGT_NM || '' - AUDIT FAILED'' ELSE TGT_NM || '' - AUDIT PASSED'' END as out_ECTL_MSG_TEXT,
...
call mplt_LKP_CTRL_TBLS_VALIDATION(''exp_UNBAL_AMT'');
...
exp_UNBAL_AMT.out_SUM_MONE_AMT_TRG as ECTL_SRC_ROW_CNT,
exp_UNBAL_AMT.out_SUM_MONE_AMT_TGT as ECTL_TRGT_INS_CNT,
exp_UNBAL_AMT.out_ECTL_UPD_CNT as ECTL_TRGT_UPD_CNT,
exp_UNBAL_AMT.out_ECTL_DEL_CNT as ECTL_TRGT_DEL_CNT,
exp_UNBAL_AMT.out_UNBAL_AMT as ECTL_TRGT_REJ_CNT,
exp_UNBAL_AMT.out_ECTL_MSG_TEXT as ECTL_MESSAGE_TXT,
exp_UNBAL_AMT.out_ECTL_INS_TS as ECTL_ORIG_INS_TS
FROM
exp_UNBAL_AMT
INNER JOIN mplt_LKP_CTRL_TBLS_VALIDATION ON exp_UNBAL_AMT.source_record_id = mplt_LKP_CTRL_TBLS_VALIDATION.source_record_id;
