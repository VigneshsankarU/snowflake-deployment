...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_MEMBER_DETAIL("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' 
...
FROM
exp_Source
LEFT JOIN m_lkp_Control_Tables ON exp_Source.source_record_id = m_lkp_Control_Tables.source_record_id
...
-- CASE WHEN ( exp_Source.TX_CD = ''NEW'' or exp_Source.TX_CD = ''UPD'' ) and ( ltrim ( rtrim ( LKP_MEMBER_DETAILS.NAME1 ) ) != ltrim ( rtrim ( exp_Source.out_NAME_1 ) ) or ltrim ( rtrim ( LKP_MEMBER_DETAILS.NAME2 ) ) != ltrim ( rtrim ( exp_Source.out_NAME_2 ) ) or ltrim ( rtrim ( LKP_MEMBER_DETAILS.ALPHA_CODE ) ) != ltrim ( rtrim ( exp_Source.out_ALPHA_CODE ) ) ) THEN TRUE ELSE FALSE END 
-- CASE WHEN exp_Source.TX_CD = ''NEW'' or ( exp_Source.TX_CD = ''UPD'' and ( LKP_MEMBER_DETAILS.NAME1 != exp_Source.out_NAME_1 or LKP_MEMBER_DETAILS.NAME2 != exp_Source.out_NAME_2 or LKP_MEMBER_DETAILS.ALPHA_CODE != exp_Source.out_ALPHA_CODE ) ) THEN TRUE ELSE FALSE END
);
...
rtr_Member_Details_UPD.source_record_id as source_record_id,
...
DATEADD ( ''d'', -1, upd_Member_Details.CHG_EFF_DT3 ) as out_EXP_DT,
...
SELECT
exp_Member_Details.MEMB_SKEY as MEMB_SKEY,
exp_Member_Details.CHG_EFF_DT as CHG_EFF_DT,
exp_Member_Details.out_EXP_DT as CHG_EXP_DT,
exp_Member_Details.NAME_1 as NAME1,
exp_Member_Details.NAME_2 as NAME2,
exp_Member_Details.ALPHA_CODE as ALPHA_CODE,
exp_Member_Details.INS_BATCH_ID as ECTL_INS_BATCH_ID,
exp_Member_Details.INS_PRGM_ID as ECTL_INS_PRGM_ID
FROM
exp_Member_Details;
