...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BASE_RECON_FASTRACK_EXTRACT("WORKLET_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS '
...
SELECT	A.MTRC, A.FRQCY,

		A.VALUE_TYPE, A.RECON_MTRC_LAYER,

		A.SOURCE_LAYER,A.TARGET_LAYER,

		A.SOURCE_VAL,A.TARGET_VAL,

		A.CMPRSN_DT, B.THRSH_VAL,

		B.THRSH_PCT, B.THRSH_VAL_CALC_MTHD,

		B.MTRC_ORIGN, B.MTRC_DESC FROM(
...
SELECT	RECON_EXTC_MTRC_INT.MTRC, RECON_EXTC_MTRC_INT.FRQCY,
		...
		MAX(RECON_EXTC_MTRC_INT.SOURCE_LAYER) AS SOURCE_LAYER, MAX(RECON_EXTC_MTRC_INT.TARGET_LAYER) AS TARGET_LAYER,

		MAX(RECON_EXTC_MTRC_INT.SOURCE_VAL) AS SOURCE_VAL, MAX(RECON_EXTC_MTRC_INT.TARGET_VAL) AS TARGET_VAL,

		RECON_EXTC_MTRC_INT.CMPRSN_DT

FROM	

 db_t_prod_core.RECON_EXTC_MTRC_INT

 WHERE RECON_MTRC_LAYER IN (''EDW | FASTTRACK'',''GW_PC | EDW'',''EDW | FAST_TRACK'',''STG_GW_PC | EDW'')

 AND RECON_EXTC_MTRC_INT.FRQCY =''QTRLY'' AND RECON_EXTC_MTRC_INT.MTRC LIKE ''FAST%''

 GROUP BY RECON_EXTC_MTRC_INT.MTRC, RECON_EXTC_MTRC_INT.FRQCY,

		RECON_EXTC_MTRC_INT.VALUE_TYPE, RECON_EXTC_MTRC_INT.RECON_MTRC_LAYER,RECON_EXTC_MTRC_INT.CMPRSN_DT

  ) A

 JOIN db_t_prod_core.RECON_MTRC_CALC B

 ON A.MTRC=B.MTRC AND A.FRQCY=B.FRQCY AND A.VALUE_TYPE=B.VALUE_TYPE AND A.RECON_MTRC_LAYER=B.RECON_MTRC_LAYER 

 AND B.RECON_MTRC_LAYER IN (''EDW | FASTTRACK'',''GW_PC | EDW'',''EDW | FAST_TRACK'',''STG_GW_PC | EDW'')

 AND B.FRQCY =''QTRLY'' AND B.MTRC LIKE ''FAST%''
) SRC
)
);
...
-- Component LKP_RECON_MTRC_FASTRACK, Type LOOKUP 
CREATE OR REPLACE TEMPORARY TABLE LKP_RECON_MTRC_FASTRACK AS
(
SELECT
LKP.MTRC,
LKP.VALUE_TYPE,
...
ROW_NUMBER() OVER(PARTITION BY exp_pass_through_data.source_record_id ORDER BY LKP.MTRC asc,LKP.VALUE_TYPE asc,LKP.RECON_MTRC_LAYER asc,LKP.DIFF_MTRC_VAL asc) RNK
FROM
exp_pass_through_data
LEFT JOIN (
SELECT A.DIFF_MTRC_VAL as DIFF_MTRC_VAL, A.MTRC as MTRC, A.VALUE_TYPE as VALUE_TYPE, A.RECON_MTRC_LAYER as RECON_MTRC_LAYER 
FROM	( 
...
ORDER	BY	LOAD_DTTM DESC, CMPRSN_DT DESC  )RNK 
		 FROM  db_t_prod_core.RECON_MTRC A  WHERE MTRC like ''FAST%'' )A WHERE A.RNK=1
...
/*REATE OR REPLACE TEMPORARY TABLE EXP_RECON_MTRC_CALC AS
(
SELECT
...
CASE WHEN exp_pass_through_data.THRSH_VAL_CALC_MTHD = ''PERCENT'' THEN ( IFNULL(TRY_TO_DECIMAL(V_SOURCE_SQL_VALUE), 0) * IFNULL(TRY_TO_DECIMAL(exp_pass_through_data.THRSH_PCT), 0) ) / 100.00 ELSE IFNULL(TRY_TO_DECIMAL(exp_pass_through_data.THRSH_VAL), 0) END as V_THRSH_VAL,
V_THRSH_VAL as O_THRSH_VAL,
TO_CHAR ( ABS ( IFNULL(TRY_TO_DECIMAL(DIFF_SQL_VAL_VAR), 0) ) - ABS ( IFNULL(TRY_TO_DECIMAL(V_THRSH_VAL), 0) ) ) as DIFF_MTRC_VAL_OVER_THRSH_VAR,
TO_CHAR ( IFNULL(TRY_TO_DECIMAL(DIFF_MTRC_VAL_OVER_THRSH_VAR), 0) ) as DIFF_MTRC_VAL_OVER_THRSH,
TO_CHAR ( IFNULL(TRY_TO_DECIMAL(TO_CHAR ( LKP_RECON_MTRC_FASTRACK.DIFF_MTRC_VAL )), 0) ) as PRIOR_DIFF_MTRC_VAL,
CASE WHEN ( IFNULL(TRY_TO_DECIMAL(DIFF_SQL_VAL_VAR), 0) - IFNULL(TRY_TO_DECIMAL(LKP_RECON_MTRC_FASTRACK.DIFF_MTRC_VAL), 0) ) <> 0 THEN CASE WHEN IFNULL(TRY_TO_DECIMAL(LKP_RECON_MTRC_FASTRACK.DIFF_MTRC_VAL), 0) <> 0 THEN TO_CHAR ( ( ( IFNULL(TRY_TO_DECIMAL(DIFF_SQL_VAL_VAR), 0) - IFNULL(TRY_TO_DECIMAL(LKP_RECON_MTRC_FASTRACK.DIFF_MTRC_VAL), 0) ) / IFNULL(TRY_TO_DECIMAL(LKP_RECON_MTRC_FASTRACK.DIFF_MTRC_VAL), 0) ) * 100 ) ELSE ''100.000'' END ELSE ''0.000'' END as PCT_CHG_DIFF_MTRC_VAL_VAR,
...
exp_pass_through_data.CMPRSN_DT as CMPRSN_DATE,
...
FROM
exp_pass_through_data
INNER JOIN LKP_RECON_MTRC_FASTRACK ON exp_pass_through_data.source_record_id = LKP_RECON_MTRC_FASTRACK.source_record_id
);
*/
...
SELECT
  ...
  CASE
    WHEN e.THRSH_VAL_CALC_MTHD = ''PERCENT'' THEN
      (TRY_TO_DECIMAL(V_SOURCE_SQL_VALUE::VARCHAR) * TRY_TO_DECIMAL(e.THRSH_PCT::VARCHAR)) / 100
    ELSE
      TRY_TO_DECIMAL(e.THRSH_VAL::VARCHAR)
  END AS V_THRSH_VAL,
  -- Output copy of threshold
  TO_CHAR(
    CASE
      WHEN e.THRSH_VAL_CALC_MTHD = ''PERCENT'' THEN
        (TRY_TO_DECIMAL(V_SOURCE_SQL_VALUE::VARCHAR) * TRY_TO_DECIMAL(e.THRSH_PCT::VARCHAR)) / 100
  ...
  TO_CHAR(
    ...
    - ABS(TRY_TO_DECIMAL(
        CASE WHEN e.THRSH_VAL_CALC_MTHD = ''PERCENT''
             THEN (TRY_TO_DECIMAL(V_SOURCE_SQL_VALUE::VARCHAR) * TRY_TO_DECIMAL(e.THRSH_PCT::VARCHAR)) / 100
             ELSE TRY_TO_DECIMAL(e.THRSH_VAL::VARCHAR)
        END
      ::VARCHAR
    )),
    ''9999999990.00''
      ...
      THEN
        CASE
          ...
          THEN ((TRY_TO_DECIMAL(DIFF_SQL_VAL_VAR::VARCHAR)
                 - TRY_TO_DECIMAL(l.DIFF_MTRC_VAL::VARCHAR))
                / TRY_TO_DECIMAL(l.DIFF_MTRC_VAL::VARCHAR)) * 100
          ELSE 100
        END
      ELSE 0
    END,
...
JOIN LKP_RECON_MTRC_FASTRACK l
  ON e.source_record_id = l.source_record_id;


-- Component RECON_MTRC_FASTRACK, Type TARGET 
...
EXP_RECON_MTRC_CALC.O_THRSH_VAL as THRSH_VAL,
