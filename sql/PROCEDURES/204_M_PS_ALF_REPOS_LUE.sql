-- Object Type: PROCEDURES
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_PS_ALF_REPOS_LUE("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' BEGIN 

-- PIPELINE START FOR 1

-- Component SQ_PS_ALF_REPOS_LUE_S, Type SOURCE 
CREATE OR REPLACE TEMPORARY TABLE SQ_PS_ALF_REPOS_LUE_S AS
(
SELECT /* adding column aliases to ensure proper downstream column references */
$1 as BUSINESS_UNIT,
$2 as TRANSACTION_ID,
$3 as DTTM_STAMP,
$4 as ALF_REC_COUNTER,
$5 as LEDGER_GROUP,
$6 as LEDGER,
$7 as JOURNAL_ID,
$8 as JOURNAL_LINE,
$9 as FISCAL_YEAR,
$10 as ACCOUNTING_PERIOD,
$11 as JOURNAL_DATE,
$12 as ACCOUNT,
$13 as DEPTID,
$14 as PRODUCT,
$15 as PROJECT_ID,
$16 as CHARTFIELD1,
$17 as CHARTFIELD2,
$18 as CHARTFIELD3,
$19 as AFFILIATE,
$20 as ALF_COV_TYPE15,
$21 as ALF_SERV_CTR_NBR,
$22 as ALF_AGENT_NBR,
$23 as ALF_POL_EFF_DATE,
$24 as ALF_POL_EXP_DATE,
$25 as ALF_POL_PREFIX,
$26 as ALF_POL_NUMBER,
$27 as ALF_COMPANY_CD,
$28 as ALF_ORIG_PREM,
$29 as ALF_LOB_CD,
$30 as ALF_TRANS_SRC_CD,
$31 as MONETARY_AMOUNT,
$32 as CURRENCY_CD,
$33 as STATISTIC_AMOUNT,
$34 as STATISTICS_CODE,
$35 as FOREIGN_AMOUNT,
$36 as FOREIGN_CURRENCY,
$37 as APPL_JRNL_ID,
$38 as BUSINESS_UNIT_GL,
$39 as ACCOUNTING_DT,
$40 as GL_DISTRIB_STATUS,
$41 as JRNL_LN_REF,
$42 as JRNL_DESCR,
$43 as PROCESS_INSTANCE,
$44 as ALF_ISSUE_ACY_TS,
$45 as source_record_id
FROM (
SELECT SRC.*, row_number() over (order by 1) AS source_record_id FROM (
SELECT BUSINESS_UNIT,

TRANSACTION_ID,

DTTM_STAMP,

ALF_REC_COUNTER,

LEDGER_GROUP,

LEDGER,

JOURNAL_ID,

JOURNAL_LINE,

FISCAL_YEAR,

ACCOUNTING_PERIOD,

JOURNAL_DATE,

ACCOUNT,

DEPTID,

PRODUCT,

PROJECT_ID,

CHARTFIELD1,

CHARTFIELD2,

CHARTFIELD3,

AFFILIATE,

ALF_COV_TYPE15,

ALF_SERV_CTR_NBR,

ALF_AGENT_NBR,

ALF_POL_EFF_DATE,

ALF_POL_EXP_DATE,

ALF_POL_PREFIX,

ALF_POL_NUMBER,

ALF_COMPANY_CD,

ALF_ORIG_PREM,

ALF_LOB_CD,

ALF_TRANS_SRC_CD,

MONETARY_AMOUNT, 

CURRENCY_CD,

STATISTIC_AMOUNT,

STATISTICS_CODE,

FOREIGN_AMOUNT,

FOREIGN_CURRENCY,

APPL_JRNL_ID,

BUSINESS_UNIT_GL,

ACCOUNTING_DT,

GL_DISTRIB_STATUS,

JRNL_LN_REF,

JRNL_DESCR,

PROCESS_INSTANCE,

ALF_ISSUE_ACY_TS   FROM DB_T_STAG_FIN_PROD.PS_ALF_REPOS_LUE_S  WHERE FISCAL_YEAR = :PSYR_ID AND ACCOUNTING_PERIOD = :PSMO_ID UNION ALL

  SELECT BUSINESS_UNIT,

TRANSACTION_ID,

DTTM_STAMP,

ALF_REC_COUNTER,

LEDGER_GROUP,

LEDGER,

JOURNAL_ID,

JOURNAL_LINE,

FISCAL_YEAR,

ACCOUNTING_PERIOD,

JOURNAL_DATE,

ACCOUNT,

DEPTID,

PRODUCT,

PROJECT_ID,

CHARTFIELD1,

CHARTFIELD2,

CHARTFIELD3,

AFFILIATE,

ALF_COV_TYPE15,

ALF_SERV_CTR_NBR,

ALF_AGENT_NBR,

ALF_POL_EFF_DATE,

ALF_POL_EXP_DATE,

ALF_POL_PREFIX,

ALF_POL_NUMBER,

ALF_COMPANY_CD,

ALF_ORIG_PREM,

ALF_LOB_CD,

ALF_TRANS_SRC_CD,

MONETARY_AMOUNT, 

CURRENCY_CD,

STATISTIC_AMOUNT,

STATISTICS_CODE,

FOREIGN_AMOUNT,

FOREIGN_CURRENCY,

APPL_JRNL_ID,

BUSINESS_UNIT_GL,

ACCOUNTING_DT,

GL_DISTRIB_STATUS,

JRNL_LN_REF,

JRNL_DESCR,

PROCESS_INSTANCE,

ALF_ISSUE_ACY_TS

    FROM PS_DB2_OWNER.PS_ALF_REPOS_LUE_A WHERE FISCAL_YEAR = :PSYR_ID AND ACCOUNTING_PERIOD = :PSMO_ID
) SRC
)
);


-- Component exp_pass_through, Type EXPRESSION 
CREATE OR REPLACE TEMPORARY TABLE exp_pass_through AS
(
SELECT
SQ_PS_ALF_REPOS_LUE_S.BUSINESS_UNIT as BUSINESS_UNIT,
SQ_PS_ALF_REPOS_LUE_S.TRANSACTION_ID as TRANSACTION_ID,
SQ_PS_ALF_REPOS_LUE_S.DTTM_STAMP as DTTM_STAMP,
SQ_PS_ALF_REPOS_LUE_S.ALF_REC_COUNTER as ALF_REC_COUNTER,
SQ_PS_ALF_REPOS_LUE_S.LEDGER_GROUP as LEDGER_GROUP,
SQ_PS_ALF_REPOS_LUE_S.LEDGER as LEDGER,
SQ_PS_ALF_REPOS_LUE_S.JOURNAL_ID as JOURNAL_ID,
SQ_PS_ALF_REPOS_LUE_S.JOURNAL_LINE as JOURNAL_LINE,
SQ_PS_ALF_REPOS_LUE_S.FISCAL_YEAR as FISCAL_YEAR,
SQ_PS_ALF_REPOS_LUE_S.ACCOUNTING_PERIOD as ACCOUNTING_PERIOD,
SQ_PS_ALF_REPOS_LUE_S.JOURNAL_DATE as JOURNAL_DATE,
SQ_PS_ALF_REPOS_LUE_S.ACCOUNT as ACCOUNT,
SQ_PS_ALF_REPOS_LUE_S.DEPTID as DEPTID,
SQ_PS_ALF_REPOS_LUE_S.PRODUCT as PRODUCT,
SQ_PS_ALF_REPOS_LUE_S.PROJECT_ID as PROJECT_ID,
SQ_PS_ALF_REPOS_LUE_S.CHARTFIELD1 as CHARTFIELD1,
SQ_PS_ALF_REPOS_LUE_S.CHARTFIELD2 as CHARTFIELD2,
SQ_PS_ALF_REPOS_LUE_S.CHARTFIELD3 as CHARTFIELD3,
SQ_PS_ALF_REPOS_LUE_S.AFFILIATE as AFFILIATE,
SQ_PS_ALF_REPOS_LUE_S.ALF_COV_TYPE15 as ALF_COV_TYPE15,
SQ_PS_ALF_REPOS_LUE_S.ALF_SERV_CTR_NBR as ALF_SERV_CTR_NBR,
SQ_PS_ALF_REPOS_LUE_S.ALF_AGENT_NBR as ALF_AGENT_NBR,
SQ_PS_ALF_REPOS_LUE_S.ALF_POL_EFF_DATE as ALF_POL_EFF_DATE,
SQ_PS_ALF_REPOS_LUE_S.ALF_POL_EXP_DATE as ALF_POL_EXP_DATE,
SQ_PS_ALF_REPOS_LUE_S.ALF_POL_PREFIX as ALF_POL_PREFIX,
SQ_PS_ALF_REPOS_LUE_S.ALF_POL_NUMBER as ALF_POL_NUMBER,
SQ_PS_ALF_REPOS_LUE_S.ALF_COMPANY_CD as ALF_COMPANY_CD,
SQ_PS_ALF_REPOS_LUE_S.ALF_ORIG_PREM as ALF_ORIG_PREM,
SQ_PS_ALF_REPOS_LUE_S.ALF_LOB_CD as ALF_LOB_CD,
SQ_PS_ALF_REPOS_LUE_S.ALF_TRANS_SRC_CD as ALF_TRANS_SRC_CD,
SQ_PS_ALF_REPOS_LUE_S.MONETARY_AMOUNT as MONETARY_AMOUNT,
SQ_PS_ALF_REPOS_LUE_S.CURRENCY_CD as CURRENCY_CD,
SQ_PS_ALF_REPOS_LUE_S.STATISTIC_AMOUNT as STATISTIC_AMOUNT,
SQ_PS_ALF_REPOS_LUE_S.STATISTICS_CODE as STATISTICS_CODE,
SQ_PS_ALF_REPOS_LUE_S.FOREIGN_AMOUNT as FOREIGN_AMOUNT,
SQ_PS_ALF_REPOS_LUE_S.FOREIGN_CURRENCY as FOREIGN_CURRENCY,
SQ_PS_ALF_REPOS_LUE_S.APPL_JRNL_ID as APPL_JRNL_ID,
SQ_PS_ALF_REPOS_LUE_S.BUSINESS_UNIT_GL as BUSINESS_UNIT_GL,
SQ_PS_ALF_REPOS_LUE_S.ACCOUNTING_DT as ACCOUNTING_DT,
SQ_PS_ALF_REPOS_LUE_S.GL_DISTRIB_STATUS as GL_DISTRIB_STATUS,
SQ_PS_ALF_REPOS_LUE_S.JRNL_LN_REF as JRNL_LN_REF,
SQ_PS_ALF_REPOS_LUE_S.JRNL_DESCR as JRNL_DESCR,
SQ_PS_ALF_REPOS_LUE_S.PROCESS_INSTANCE as PROCESS_INSTANCE,
SQ_PS_ALF_REPOS_LUE_S.source_record_id
FROM
SQ_PS_ALF_REPOS_LUE_S
);


-- Component PS_ALF_REPOS_LUE_S, Type TARGET 
INSERT INTO DB_T_STAG_FIN_PROD.PS_ALF_REPOS_LUE_S
(
BUSINESS_UNIT,
TRANSACTION_ID,
DTTM_STAMP,
ALF_REC_COUNTER,
LEDGER_GROUP,
LEDGER,
JOURNAL_ID,
JOURNAL_LINE,
FISCAL_YEAR,
ACCOUNTING_PERIOD,
JOURNAL_DATE,
ACCOUNT_CD,
DEPTID,
PRODUCT,
PROJECT_ID,
CHARTFIELD1,
CHARTFIELD2,
CHARTFIELD3,
AFFILIATE,
ALF_COV_TYPE15,
ALF_SERV_CTR_NBR,
ALF_AGENT_NBR,
ALF_POL_EFF_DATE,
ALF_POL_EXP_DATE,
ALF_POL_PREFIX,
ALF_POL_NUMBER,
ALF_COMPANY_CD,
ALF_ORIG_PREM,
ALF_LOB_CD,
ALF_TRANS_SRC_CD,
MONETARY_AMOUNT,
CURRENCY_CD,
STATISTIC_AMOUNT,
STATISTICS_CODE,
FOREIGN_AMOUNT,
FOREIGN_CURRENCY,
APPL_JRNL_ID,
BUSINESS_UNIT_GL,
ACCOUNTING_DT,
GL_DISTRIB_STATUS,
JRNL_LN_REF,
JRNL_DESCR,
PROCESS_INSTANCE
)
SELECT
exp_pass_through.BUSINESS_UNIT as BUSINESS_UNIT,
exp_pass_through.TRANSACTION_ID as TRANSACTION_ID,
exp_pass_through.DTTM_STAMP as DTTM_STAMP,
exp_pass_through.ALF_REC_COUNTER as ALF_REC_COUNTER,
exp_pass_through.LEDGER_GROUP as LEDGER_GROUP,
exp_pass_through.LEDGER as LEDGER,
exp_pass_through.JOURNAL_ID as JOURNAL_ID,
exp_pass_through.JOURNAL_LINE as JOURNAL_LINE,
exp_pass_through.FISCAL_YEAR as FISCAL_YEAR,
exp_pass_through.ACCOUNTING_PERIOD as ACCOUNTING_PERIOD,
exp_pass_through.JOURNAL_DATE as JOURNAL_DATE,
exp_pass_through.ACCOUNT as ACCOUNT_CD,
exp_pass_through.DEPTID as DEPTID,
exp_pass_through.PRODUCT as PRODUCT,
exp_pass_through.PROJECT_ID as PROJECT_ID,
exp_pass_through.CHARTFIELD1 as CHARTFIELD1,
exp_pass_through.CHARTFIELD2 as CHARTFIELD2,
exp_pass_through.CHARTFIELD3 as CHARTFIELD3,
exp_pass_through.AFFILIATE as AFFILIATE,
exp_pass_through.ALF_COV_TYPE15 as ALF_COV_TYPE15,
exp_pass_through.ALF_SERV_CTR_NBR as ALF_SERV_CTR_NBR,
exp_pass_through.ALF_AGENT_NBR as ALF_AGENT_NBR,
exp_pass_through.ALF_POL_EFF_DATE as ALF_POL_EFF_DATE,
exp_pass_through.ALF_POL_EXP_DATE as ALF_POL_EXP_DATE,
exp_pass_through.ALF_POL_PREFIX as ALF_POL_PREFIX,
exp_pass_through.ALF_POL_NUMBER as ALF_POL_NUMBER,
exp_pass_through.ALF_COMPANY_CD as ALF_COMPANY_CD,
exp_pass_through.ALF_ORIG_PREM as ALF_ORIG_PREM,
exp_pass_through.ALF_LOB_CD as ALF_LOB_CD,
exp_pass_through.ALF_TRANS_SRC_CD as ALF_TRANS_SRC_CD,
exp_pass_through.MONETARY_AMOUNT as MONETARY_AMOUNT,
exp_pass_through.CURRENCY_CD as CURRENCY_CD,
exp_pass_through.STATISTIC_AMOUNT as STATISTIC_AMOUNT,
exp_pass_through.STATISTICS_CODE as STATISTICS_CODE,
exp_pass_through.FOREIGN_AMOUNT as FOREIGN_AMOUNT,
exp_pass_through.FOREIGN_CURRENCY as FOREIGN_CURRENCY,
exp_pass_through.APPL_JRNL_ID as APPL_JRNL_ID,
exp_pass_through.BUSINESS_UNIT_GL as BUSINESS_UNIT_GL,
exp_pass_through.ACCOUNTING_DT as ACCOUNTING_DT,
exp_pass_through.GL_DISTRIB_STATUS as GL_DISTRIB_STATUS,
exp_pass_through.JRNL_LN_REF as JRNL_LN_REF,
exp_pass_through.JRNL_DESCR as JRNL_DESCR,
exp_pass_through.PROCESS_INSTANCE as PROCESS_INSTANCE
FROM
exp_pass_through;


-- PIPELINE END FOR 1

-- PIPELINE START FOR 2

-- Component SQ_PS_ALF_REPOS_LUE_S1, Type SOURCE 
CREATE OR REPLACE TEMPORARY TABLE SQ_PS_ALF_REPOS_LUE_S1 AS
(
SELECT /* adding column aliases to ensure proper downstream column references */
$1 as CNT,
$2 as MONETARY_AMOUNT,
$3 as source_record_id
FROM (
SELECT SRC.*, row_number() over (order by 1) AS source_record_id FROM (
with c AS (select count(1) CNT FROM PS_ALF_REPOS_LUE_S) 

 ,s AS (SELECT  COALESCE(SUM(CAST(MONETARY_AMOUNT AS FLOAT)), 0) AS MONETARY_AMOUNT

FROM DB_T_STAG_FIN_PROD.PS_ALF_REPOS_LUE_S

WHERE FISCAL_YEAR =  :PSYR_ID

AND ACCOUNTING_PERIOD = :PSMO_ID

and  ACCOUNT_CD IN (SELECT ACCT_NBR FROM DB_T_SHRD_FIN_PROD.REPOS_FIN_ACCT_NBR_LKUP           

WHERE REPOS_CD = ''LUE'')

AND GL_DISTRIB_STATUS = ''D'')

select  C.CNT,S.MONETARY_AMOUNT

from

C join S

on 1=1
) SRC
)
);


-- Component exp_ps_alf_repos_lue, Type EXPRESSION 
CREATE OR REPLACE TEMPORARY TABLE exp_ps_alf_repos_lue AS
(
SELECT
SQ_PS_ALF_REPOS_LUE_S1.CNT as record_count,
:feed_ind as feed_ind,
DATE_PART(''MM'', TO_TIMESTAMP(CURRENT_TIMESTAMP)) as feed_mm,
CASE WHEN feed_mm = 1 THEN 12 ELSE ( feed_mm - 1 ) END as o_feed_mm,
DATE_PART(''YY'', TO_TIMESTAMP(CURRENT_TIMESTAMP)) as feed_yy,
CASE WHEN feed_mm = 1 THEN feed_yy - 1 ELSE feed_yy END as o_feed_yy,
o_feed_yy || ''-'' || LPad ( o_feed_mm , 2 , ''0'' ) || ''-'' || ''01'' as feed_dt,
to_char ( SQ_PS_ALF_REPOS_LUE_S1.MONETARY_AMOUNT ) as o_MONETARY_AMOUNT,
FIELD1,
FIELD2,
FIELD3,
FIELD4,
FIELD5,
FIELD6,
FIELD7,
FIELD8,
FIELD9,
FIELD10,
FIELD11,
FIELD12,
FIELD13,
FIELD14,
FIELD15,
FIELD16,
FIELD17,
FIELD18,
FIELD19,
FIELD20,
FIELD21,
FIELD22,
FIELD23,
FIELD24,
FIELD25,
FIELD26,
SQ_PS_ALF_REPOS_LUE_S1.source_record_id
FROM
SQ_PS_ALF_REPOS_LUE_S1
);


-- Component TRG_STAG_PS_REPOS, Type TARGET_EXPORT_PREPARE Stage data before exporting
CREATE OR REPLACE TEMPORARY TABLE TRG_STAG_PS_REPOS AS
(
SELECT
exp_ps_alf_repos_lue.feed_ind as feed_ind,
exp_ps_alf_repos_lue.feed_dt as feed_dt,
exp_ps_alf_repos_lue.record_count as record_cnt,
exp_ps_alf_repos_lue.o_MONETARY_AMOUNT as monetary_amount,
exp_ps_alf_repos_lue.FIELD1 as FIELD1,
exp_ps_alf_repos_lue.FIELD2 as FIELD2,
exp_ps_alf_repos_lue.FIELD3 as FIELD3,
exp_ps_alf_repos_lue.FIELD4 as FIELD4,
exp_ps_alf_repos_lue.FIELD5 as FIELD5,
exp_ps_alf_repos_lue.FIELD6 as FIELD6,
exp_ps_alf_repos_lue.FIELD7 as FIELD7,
exp_ps_alf_repos_lue.FIELD8 as FIELD8,
exp_ps_alf_repos_lue.FIELD9 as FIELD9,
exp_ps_alf_repos_lue.FIELD10 as FIELD10,
exp_ps_alf_repos_lue.FIELD11 as FIELD11,
exp_ps_alf_repos_lue.FIELD12 as FIELD12,
exp_ps_alf_repos_lue.FIELD13 as FIELD13,
exp_ps_alf_repos_lue.FIELD14 as FIELD14,
exp_ps_alf_repos_lue.FIELD15 as FIELD15,
exp_ps_alf_repos_lue.FIELD16 as FIELD16,
exp_ps_alf_repos_lue.FIELD17 as FIELD17,
exp_ps_alf_repos_lue.FIELD18 as FIELD18,
exp_ps_alf_repos_lue.FIELD19 as FIELD19,
exp_ps_alf_repos_lue.FIELD20 as FIELD20,
exp_ps_alf_repos_lue.FIELD21 as FIELD21,
exp_ps_alf_repos_lue.FIELD22 as FIELD22,
exp_ps_alf_repos_lue.FIELD23 as FIELD23,
exp_ps_alf_repos_lue.FIELD24 as FIELD24,
exp_ps_alf_repos_lue.FIELD25 as FIELD25,
exp_ps_alf_repos_lue.FIELD26 as FIELD26
FROM
exp_ps_alf_repos_lue
);


-- Component TRG_STAG_PS_REPOS, Type EXPORT_DATA Exporting data
;


-- PIPELINE END FOR 2

END; ';