...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_AUDIT_BENFIELD_TABLES("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
...
SELECT COALESCE(COUNT(*),0)  TGT_CNT,''1'' AS DUMMY FROM table(:TGT_NM) WHERE 1=1
--TGT_FLTR_VAR
) SRC
)
);
...
(
SELECT
TO_NUMBER(LTRIM ( RTRIM ( sq_TGT_TABLE.TGT_CNT ) )) as out_TGT_CNT,
...
FROM (
SELECT SRC.*, row_number() over (order by 1) AS source_record_id FROM (
SELECT COUNT(*) SRC_CNT,''1'' AS DUMMY FROM table(:SRC_NM) GROUP BY 2
) SRC
)
);
...
exp_INT_CONV_TGT_TABLE.out_TGT_CNT as TGT_CNT,
exp_INT_CONV_TGT_TABLE.out_DUMMY as DUMMY_TGT,
row_number() over (order by 1) AS source_record_id
FROM
exp_INT_CONV_TGT_TABLE
INNER JOIN exp_INT_CON_SRC_TABLE ON exp_INT_CONV_TGT_TABLE.out_DUMMY = exp_INT_CON_SRC_TABLE.out_DUMMY
);
...
jnr_SRC_TGT_TABLE.TGT_CNT as TARGET_TGT,
jnr_SRC_TGT_TABLE.SRC_CNT - jnr_SRC_TGT_TABLE.TGT_CNT as TARGET_REJ,
0 as TARGET_UPD,
0 as TARGET_DEL,
...
exp_MSG_DESC.TARGET_TGT as ECTL_TRGT_INS_CNT,
