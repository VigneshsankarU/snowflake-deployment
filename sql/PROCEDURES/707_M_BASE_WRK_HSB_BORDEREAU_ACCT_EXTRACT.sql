...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_BASE_WRK_HSB_BORDEREAU_ACCT_EXTRACT("WORKLET_NAME" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS ' 
...
COMPANY_PROD_ID :=   (SELECT param_value FROM control_params where run_id = :run_id and upper(param_name)=''COMPANY_PROD_ID'' order by insert_ts desc limit 1);
...
SELECT	HSB_BORDEREAU.TRANS_ID, HSB_BORDEREAU.PROD_CD, HSB_BORDEREAU.PROD_ID,

		HSB_BORDEREAU.POL_NBR, HSB_BORDEREAU.TRANS_TYPE, HSB_BORDEREAU.TRANS_EFF_DT,

		HSB_BORDEREAU.POL_EFF_DT, HSB_BORDEREAU.POL_TERM_EXP_DT, HSB_BORDEREAU.TRANS_PREM_GROSS,

		HSB_BORDEREAU.TRANS_PREM_NET, HSB_BORDEREAU.PREV_POL_NBR, HSB_BORDEREAU.PROG_ID,

/* EIM-21057-Start */
/* HSB_BORDEREAU.NAMED_INSRD, */
		CASE 

			WHEN HSB_BORDEREAU.NAMED_INSRD LIKE ''%,%'' THEN REPLACE(HSB_BORDEREAU.NAMED_INSRD,

		'','','''') 

			ELSE HSB_BORDEREAU.NAMED_INSRD 

		END AS NAMED_INSRD,

/* EIM-21057-End */
HSB_BORDEREAU.CNTRCT_NUM, HSB_BORDEREAU.UW_CMPNY,

		HSB_BORDEREAU.POL_RSK_ST, HSB_BORDEREAU.POL_TYPE_DESC, 

HSB_BORDEREAU.MAIL_CITY,

		HSB_BORDEREAU.MAIL_ST, HSB_BORDEREAU.MAIL_ZIP, HSB_BORDEREAU.HSB_BRANCH_CD,

		HSB_BORDEREAU.TOT_PROP_PREM, HSB_BORDEREAU.TRANS_ENTRY_DT, HSB_BORDEREAU.COVERAGE_CD,

		HSB_BORDEREAU.COMMISSION_RATE, HSB_BORDEREAU.EB_TREATY_RATE 

FROM

 DB_T_PROD_WRK.HSB_BORDEREAU



WHERE

/*            CAST(HSB_BORDEREAU.TRANS_EFF_DT AS DATE)  >= :CAL_START_DT */
/*  AND CAST(HSB_BORDEREAU.TRANS_EFF_DT AS DATE)  <= :CAL_END_DT */


  CASE 

    WHEN cast(HSB_BORDEREAU.TRANS_ENTRY_DT AS DATE )  > CAST          (HSB_BORDEREAU.TRANS_EFF_DT AS DATE) 

         THEN cast(HSB_BORDEREAU.TRANS_ENTRY_DT AS DATE)

     ELSE cast(HSB_BORDEREAU.TRANS_EFF_DT AS DATE)
...
RPAD ( SQ_src_HSB_BORDEREAU.PROD_ID , 4 , '' '' ) as out_PROD_ID,
...
RPAD ( var_SPACES , 3 , '' '' ) as out_COVERAGE_CD,
...
DECODE ( true , SUBSTR ( v_POL_TYPE_DESC , 1 , 2 ) = ''HO'' , ''HO'' , SUBSTR ( v_POL_TYPE_DESC , 1 , 2 ) = ''MH'' , ''MH'' , SUBSTR ( v_POL_TYPE_DESC , 1 , 3 ) = ''FOP'' , ''FOP'' , ''NA'' ) as out_Lob_cd,
...
EXPTRANS.out_EB_TREATY_RATE as EB_Treaty_Rate,
...
:COMPANY_PROD_ID as C1_Company_Product_ID,
C1_Company_Product_ID as Out_C1_Company_Product_ID,
