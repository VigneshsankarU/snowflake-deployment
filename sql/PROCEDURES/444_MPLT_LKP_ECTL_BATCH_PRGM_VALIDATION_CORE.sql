...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.MPLT_LKP_ECTL_BATCH_PRGM_VALIDATION_CORE("IN_IN_MPLT_LKP_BATCH_PRGM" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS '
...
EXECUTE IMMEDIATE ''CREATE OR REPLACE TEMPORARY TABLE exp_HARDCODE_BATCH_ACTIVE_IND AS
(
    SELECT
...
(
    ...
    CASE WHEN LKP_ECTL_BATCH_INFO.ECTL_BATCH_ID IS NULL OR LKP_ECTL_PRGM_INFO.ECTL_PRGM_ID IS NULL 
        THEN ''ERROR-BATCH/PROGRAM NOT ACTIVE'' --TO_NUMBER(''ERROR-BATCH/PROGRAM NOT ACTIVE'') 
        ELSE NULL 
