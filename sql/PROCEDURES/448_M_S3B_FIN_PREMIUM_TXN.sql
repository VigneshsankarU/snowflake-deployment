...
CREATE OR REPLACE PROCEDURE ALFA_EDW_DEV.PUBLIC.M_S3B_FIN_PREMIUM_TXN("RUN_ID" VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
...
PMSTG_FIN_S3BTABLENAME varchar;
REPOS_CD varchar;
BEGIN 

PMMAPPINGNAME:='' ''; 
PMSTG_FIN_S3BTABLENAME:='' ''; 
REPOS_CD:='' ''; 
...
-- Component sq_STG_FIN_S3B, Type SOURCE 
CREATE OR REPLACE TEMPORARY TABLE sq_STG_FIN_S3B AS
(
...
$19 as ALF_COV_TYPE,
$20 as ALF_ACCOUNT_ID,
$21 as source_record_id
FROM (
SELECT SRC.*, row_number() over (order by 1) AS source_record_id FROM (
SELECT TRIM(STG_FIN_S3B.BUSINESS_UNIT), TRIM(STG_FIN_S3B.LEDGER), STG_FIN_S3B.FISCAL_YEAR , STG_FIN_S3B.ACCOUNTING_PERIOD , TRIM(STG_FIN_S3B.ACCOUNT_NBR), TRIM(STG_FIN_S3B.DEPTID), TRIM(STG_FIN_S3B.PRODUCT), TRIM(STG_FIN_S3B.PROJECT_ID), TRIM(STG_FIN_S3B.CHARTFIELD1), TRIM(STG_FIN_S3B.CHARTFIELD2), TRIM(STG_FIN_S3B.CHARTFIELD3), TRIM(STG_FIN_S3B.ALF_POL_PREFIX), TRIM(STG_FIN_S3B.ALF_POLICY_NBR), TRIM(STG_FIN_S3B.ALF_AGT_NBR), TRIM(STG_FIN_S3B.ALF_SERV_CTR_NBR), TRIM(STG_FIN_S3B.ALF_COMPANY_CD), TRIM(STG_FIN_S3B.GL_DISTRIB_STATUS), STG_FIN_S3B.MONETARY_AMOUNT,
TRIM(STG_FIN_S3B.ALF_COV_TYPE),
TRIM(STG_FIN_S3B.ALF_ACCOUNT_ID)
FROM
DB_T_STAG_FIN_PROD.STG_FIN_S3B
WHERE
GL_DISTRIB_STATUS=''D''
) SRC
)
...
(
SELECT
substr ( sq_STG_FIN_S3B.BUSINESS_UNIT , 1 , 3 ) as CMPY_CD,
sq_STG_FIN_S3B.CHARTFIELD1 as ST_CD,
CASE WHEN try_cast(sq_STG_FIN_S3B.ALF_SERV_CTR_NBR as int)>0 THEN TO_CHAR ( TO_NUMBER(sq_STG_FIN_S3B.ALF_SERV_CTR_NBR) ) ELSE sq_STG_FIN_S3B.ALF_SERV_CTR_NBR END as out_SVC_NBR,
CASE WHEN try_cast(sq_STG_FIN_S3B.ALF_AGENT_NO as int)>0 THEN TO_CHAR ( TO_NUMBER(sq_STG_FIN_S3B.ALF_AGENT_NO) ) ELSE sq_STG_FIN_S3B.ALF_AGENT_NO END as out_AGNT_NBR,
sq_STG_FIN_S3B.ALF_COMPANY_CD as CMPY_TYPE_CD,
CASE WHEN LENGTH ( LTRIM ( RTRIM ( sq_STG_FIN_S3B.ALF_POL_PREFIX ) ) ) = 1 THEN LTRIM ( RTRIM ( sq_STG_FIN_S3B.ALF_POL_PREFIX ) ) || ''00000'' ELSE LTRIM ( RTRIM ( sq_STG_FIN_S3B.ALF_POL_PREFIX ) ) || ''0000'' END as var_PRODUCT_CD,
CASE WHEN LTRIM ( RTRIM ( sq_STG_FIN_S3B.PRODUCT ) ) IS NULL OR LTRIM ( RTRIM ( sq_STG_FIN_S3B.PRODUCT ) ) = '''' THEN ''A00000'' ELSE CASE WHEN LTRIM ( RTRIM ( sq_STG_FIN_S3B.PRODUCT ) ) = ''NO_PRD'' THEN CASE WHEN sq_STG_FIN_S3B.ALF_POL_PREFIX IN ( LTRIM ( RTRIM ( sq_STG_FIN_S3B.ALF_POL_PREFIX ) ) , ''G'' , ''N'' ) = 1 THEN ''A00000'' ELSE CASE WHEN LTRIM ( RTRIM ( sq_STG_FIN_S3B.ALF_POL_PREFIX ) ) = ''B'' THEN ''C00001'' ELSE var_PRODUCT_CD END END ELSE sq_STG_FIN_S3B.PRODUCT END END as out_PRODUCT_CD,
sq_STG_FIN_S3B.ALF_COV_TYPE as ALF_COV_TYPE,
sq_STG_FIN_S3B.ALF_POL_NUMBER as PLCY_NBR,
NULL as PLCY_EFF_DT,
NULL as PLCY_EXP_DT,
NULL as PLCY_TERM,
sq_STG_FIN_S3B.ACCOUNTING_PERIOD as ACCTNG_MO,
sq_STG_FIN_S3B.FISCAL_YEAR as ACCTNG_YR,
sq_STG_FIN_S3B.FISCAL_YEAR * 100 + sq_STG_FIN_S3B.ACCOUNTING_PERIOD as MO_ID,
sq_STG_FIN_S3B.CHARTFIELD2 as POOL_CD,
sq_STG_FIN_S3B.CHARTFIELD3 as REINSUR_CD,
0 as TOT_PREM_AMT,
sq_STG_FIN_S3B.MONETARY_AMOUNT as MONETARY_AMT,
sq_STG_FIN_S3B.LEDGER as LEDGER,
sq_STG_FIN_S3B.ACCOUNT_NBR as ACCT_NBR,
sq_STG_FIN_S3B.GL_DISTRIB_STATUS as GL_DSTR_STATUS,
sq_STG_FIN_S3B.DEPTID as DEPT_ID,
sq_STG_FIN_S3B.PROJECT_ID as PROJ_ID,
sq_STG_FIN_S3B.ALF_ACCOUNT_ID as ALF_ACCT_ID,
:PMMappingName as out_PRGM_NM,
:ACT_PROJ_ID as out_ACT_PROJ_ID,
:ECTL_ACTIVE_IND as out_BATCH_ACTIVE_IND,
:PMSTG_FIN_S3BTableName as out_TABLE_NM,
:REPOS_CD as out_REPOS_CD,
:ACT_PROJ_ID as IN_ACT_PROJ_ID,
:ECTL_ACTIVE_IND as IN_BATCH_ACTIVE_IND,
''m_STG_FIN_CLM'' IN_PRGM_NM,
null IN_TABLE_NM,
sq_STG_FIN_S3B.source_record_id
FROM
sq_STG_FIN_S3B
);
...
exp_GET_DATA
LEFT JOIN (
SELECT FIN_PREMIUM_TXN.CMPY_CD as CMPY_CD, FIN_PREMIUM_TXN.ST_CD as ST_CD, FIN_PREMIUM_TXN.SVC_NBR as SVC_NBR, FIN_PREMIUM_TXN.AGNT_NBR as AGNT_NBR, FIN_PREMIUM_TXN.CMPY_TYPE_CD as CMPY_TYPE_CD, FIN_PREMIUM_TXN.PRODUCT_CD as PRODUCT_CD, 
FIN_PREMIUM_TXN.COV_TYPE_CD as COV_TYPE_CD,
FIN_PREMIUM_TXN.PLCY_NBR as PLCY_NBR, FIN_PREMIUM_TXN.PLCY_EFF_DT as PLCY_EFF_DT, FIN_PREMIUM_TXN.PLCY_EXP_DT as PLCY_EXP_DT, FIN_PREMIUM_TXN.ACCTNG_MO as ACCTNG_MO, FIN_PREMIUM_TXN.ACCTNG_YR as ACCTNG_YR, FIN_PREMIUM_TXN.POOL_CD as POOL_CD, FIN_PREMIUM_TXN.REINSUR_CD as REINSUR_CD, FIN_PREMIUM_TXN.LEDGER as LEDGER, FIN_PREMIUM_TXN.ACCT_NBR as ACCT_NBR, FIN_PREMIUM_TXN.GL_DSTR_STATUS as GL_DSTR_STATUS, FIN_PREMIUM_TXN.DEPT_ID as DEPT_ID,
FIN_PREMIUM_TXN.ALF_ACCT_ID as ALF_ACCT_ID
...
) LKP ON LKP.CMPY_CD = exp_GET_DATA.CMPY_CD AND LKP.ST_CD = exp_GET_DATA.ST_CD AND LKP.SVC_NBR = exp_GET_DATA.out_SVC_NBR AND LKP.AGNT_NBR = exp_GET_DATA.out_AGNT_NBR AND LKP.CMPY_TYPE_CD = exp_GET_DATA.CMPY_TYPE_CD AND LKP.PRODUCT_CD = exp_GET_DATA.out_PRODUCT_CD AND LKP.PLCY_NBR = exp_GET_DATA.PLCY_NBR AND LKP.PLCY_EFF_DT = exp_GET_DATA.PLCY_EFF_DT AND LKP.PLCY_EXP_DT = exp_GET_DATA.PLCY_EXP_DT AND LKP.ACCTNG_MO = exp_GET_DATA.ACCTNG_MO AND LKP.ACCTNG_YR = exp_GET_DATA.ACCTNG_YR AND LKP.POOL_CD = exp_GET_DATA.POOL_CD AND LKP.REINSUR_CD = exp_GET_DATA.REINSUR_CD AND LKP.LEDGER = exp_GET_DATA.LEDGER AND LKP.ACCT_NBR = exp_GET_DATA.ACCT_NBR AND LKP.GL_DSTR_STATUS = exp_GET_DATA.GL_DSTR_STATUS AND LKP.DEPT_ID = exp_GET_DATA.DEPT_ID AND LKP.ALF_ACCT_ID = exp_GET_DATA.ALF_ACCT_ID AND LKP.COV_TYPE_CD = exp_GET_DATA.ALF_COV_TYPE
QUALIFY RNK = 1
);
...
in_TOT_PREM_AMT as TOT_PREM_AMT,
...
-- Component FIN_PREMIUM_TXN_S3B_INS, Type TARGET 
